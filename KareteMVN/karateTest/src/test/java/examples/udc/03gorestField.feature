Feature: Check returned users by API
  ...
  Scenario: get user with id 7843384 by param
    Given path '/public-api/users'
    And param id = '7843384'
    When method GET
    Then status 200
    And print response
    And match $.data[0] contains {id:"#number",name:"#string",email:"#regex [A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,9}",gender: "#notnull"}
